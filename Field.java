...
public class Field {
    ...
    public List<Cell> getLivingNeighbours(Location location) {
      assert location != null : "Null location passed to getLivingNeighbours";
    ...
    /**
     * Returns the number of Brucella cells in a row
     * @param the row number
     * @return the number of brucella cells in a row
     */
    public int numberBrucellaInRow(int row){
        int num = 0;
        for (int i = 0; i < SimulatorView.GRID_WIDTH; i++){
            if (field[row][i] instanceof Brucella){ //check if the cell is a brucella cell (either living or dead)
    ...
    /**
     * Kills the entire row of cells
     * (used in Brucella class)
     * @param the row number
     */
    public void wipeOutRowofCell(int row) {
        for (int i = 0; i < SimulatorView.GRID_WIDTH; i++){
            if (!(field[row][i] instanceof Brucella)){ //ensures that only non-brucella cells die
                field[row][i].setNextState(false); //dies
            }
        }
    }
    
    /**
     * Kills all neighbours near it
     * (used in Ureaplasma class)
     * @param location Get locations adjacent to this.
     */
    public void wipeOutNeighbours(Location location) {
        assert location != null : "Null location passed to wipeOutNeighbours";
        if (location != null) {
        List<Location> adjLocations = adjacentLocations(location);
            for (Location loc : adjLocations) {
              Cell cell = field[loc.getRow()][loc.getCol()];
              if (cell.isAlive()){
                  cell.setNextState(false); //dies
              }
            }
      }
    }
